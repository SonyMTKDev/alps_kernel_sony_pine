...
#define DBGCONTROL_TZ_ASSERT	0x82000284
...
//tz crash
static int generate_tz_crash_show(struct seq_file *m, void *v)
{
	seq_printf(m,"%s \n","tz crash Generated!\n");
#ifdef CONFIG_ARM64
	dbgcontrol_secure_call(DBGCONTROL_TZ_ASSERT, (u64)0, 0, 0);
#else
#ifdef CONFIG_ARM_PSCI
	dbgcontrol_secure_call(DBGCONTROL_TZ_ASSERT, (u32)0, 0, 0);
#endif
#endif
	return 0;
}

static int proc_generate_tz_crash_open(struct inode *inode, struct file *file)
{
	return single_open(file, generate_tz_crash_show, NULL);
}

static int generate_kernel_api_show(struct seq_file *m, void *v)
{
	seq_printf(m,"%s \n","kernel api Generated!\n");
	//aee_kernel_exception_api(const char *file, const int line, const int db_opt,
	//		      const char *module, const char *msg, ...)
	aee_kernel_exception("kernel_api", "We trigger Kernel API DB");

	return 0;
}

static int proc_generate_kernel_api_open(struct inode *inode, struct file *file)
{
	return single_open(file, generate_kernel_api_show, NULL);
}
...
ADD_FILE_OPS(generate_tz_crash);
ADD_FILE_OPS(generate_kernel_api);
...
static int __init dbgcontrol_init(void)
{
	...
	if (debug_feature_enable) {
		...
		ADD_PROC_ENTRY(generate-tz-crash, generate_tz_crash, S_IRUSR | S_IWUSR);
		ADD_PROC_ENTRY(generate-kernel-api, generate_kernel_api, S_IRUSR | S_IWUSR);
...
static void __exit dbgcontrol_deinit( void )
{
	...
	if (debug_feature_enable) {
		...
		remove_proc_entry("generate-tz-crash", dbgcontrol_proc_dir);
		remove_proc_entry("get-status", dbgcontrol_proc_dir);
		remove_proc_entry("generate-kernel-api", dbgcontrol_proc_dir);
