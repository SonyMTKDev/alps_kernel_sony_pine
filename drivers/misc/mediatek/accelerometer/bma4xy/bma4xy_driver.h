...
/* generic */
#define BMA4XY_CHIP_ID (0x1F)
#define CHECK_CHIP_ID_TIME_MAX   5
#define FIFO_DATA_BUFSIZE_BMA4XY    1024
#define FIRMWARE_DATA_SIZE     6144
#define BMA4XY_ENABLE_INT1  1
#define BMA4XY_REG_NAME(name) BMA4XY_##name
#define BMA4XY_VAL_NAME(name) BMA4XY_##name
#define BMA4XY_CALL_API(name) bma4xy_##name
...
//<--[SM31][Sensors][JasonHsing] Modify G-sensor driver to reduce kernel initial time 20161020 BEGIN --
	struct work_struct load_config;
...
//<--[SM31][Sensors][JasonHsing] Modify G-sensor driver to reduce kernel initial time 20161020 BEGIN --
	struct delayed_work delay_load_config;
//-->[SM31][Sensors][JasonHsing] Modify G-sensor driver to reduce kernel initial time 20161230 END --
	atomic_t in_suspend;
	uint8_t tap_type;
	uint8_t selftest;
...
extern struct bma4xy_client_data *obj_i2c_data;
