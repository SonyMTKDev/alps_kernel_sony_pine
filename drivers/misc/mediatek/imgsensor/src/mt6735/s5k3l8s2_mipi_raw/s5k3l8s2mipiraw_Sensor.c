...
#include "s5k3l8s2mipiraw_Sensor.h"
...
extern int read_s5k3l8_otp_mtk_fmt(void);

static kal_uint8 s5k3l8s2_Module_id;//ModuleId+VcmId
...
#define LOG_1 LOG_INF("S5K3L8S2,MIPI 4LANE\n")\
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3L8S2_SENSOR_ID,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
	
	.checksum_value = 0x49c09f86,		//checksum value for Camera Auto Test
	
	.pre = {
		.pclk = 480000000,		//560000000,				//record different mode's pclk
		.linelength  = 5808,				//record different mode's linelength
		.framelength = 2756,   // 3206,			//record different mode's framelength
...
//$MV1[MCLK:24,Width:2104,Height:1560,Format:MIPI_Raw10,mipi_lane:4,mipi_datarate:1124,pvi_pclk_inverse:0]
  ...
  write_cmos_sensor(0x0306, 0x0096); //0x00AF
  write_cmos_sensor(0x0302, 0x0001);
  write_cmos_sensor(0x0300, 0x0005);
  write_cmos_sensor(0x030C, 0x0006);
  write_cmos_sensor(0x030E, 0x0096); //0x0119
  ...
  #endif
  write_cmos_sensor(0x0340, 0x0AC4); //0x0C86
...
write_cmos_sensor(0x0B08, 0x0100);	//D.BPC_Off
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		LOG_INF("imgsensor 0x%x sensor_id = 0x%x",imgsensor_info.sensor_id,*sensor_id);
		if(*sensor_id == 0x30C8)
			s5k3l8s2_Module_id = read_s5k3l8_otp_mtk_fmt();		

		if((*sensor_id == 0x30C8) && (s5k3l8s2_Module_id != 0x61))
			{
				if(s5k3l8s2_Module_id == 0x0A)
					*sensor_id = 0x30C9;
				else//0X0E (Module ID 0x02+VCM ID 0x0C)
					*sensor_id = 0x30CA;
				LOG_INF("New Sensor ID = 0x%x \n",*sensor_id);
			}
