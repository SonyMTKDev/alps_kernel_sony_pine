...
#define S5K5E2YA_MODULE_ID 0x57
...
// [SM31][Camera][akenhsu] Add OTP for front camera: s5k5e2ya 20161004 BEGIN
...
void S5K5E2YA_MIPI_read_otp_wb(struct S5K5E2YA_MIPI_otp_struct *otp)
{
	...
	for(PageCount = 3; PageCount>=2; PageCount--)
	{
		...
		if(read_cmos_sensor(0x0a24) == 0x01)
		{
			...
			if (Structure_Version == 1) // Its AWB Golden data is wrong. Assign the correct one. 20161019 Joseph Hsu
			{
				otp->golden_awb_r  = 0x4d;
				otp->golden_awb_gr = 0x67;
				otp->golden_awb_gb = 0x66;
				otp->golden_awb_b  = 0x3e;
			}
	...
	LOG_INF("[S5K5E2YA] Structure_Version=0x%x\n", Structure_Version);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == 0x5e20 && module_id == S5K5E2YA_MODULE_ID) {
				LOG_INF("Found S5K5E2 YA module! \n");
				*sensor_id = 0x5e20;
				break;
			} else {
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == 0x5e20) {
				if (module_id == S5K5E2YA_MODULE_ID) {
					LOG_INF("Found S5K5E2 YA module! \n");
					sensor_id = 0x5e20;
					LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
					break;
	...
	if (module_id != S5K5E2YA_MODULE_ID) return ERROR_INVALID_DRIVER_MOD_ID;
