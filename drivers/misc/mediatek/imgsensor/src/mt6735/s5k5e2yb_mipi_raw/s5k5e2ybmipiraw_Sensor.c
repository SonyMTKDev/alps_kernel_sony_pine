...
#define S5K5E2YB_MODULE_ID 0x42
/****************************Modify following Strings for debug****************************/
#define PFX "s5k5e2yb_camera_sensor"
#define LOG_1 LOG_INF("s5k5e2yb,MIPI 2LANE\n")
...
// [SM31][Camera][akenhsu] Add OTP for front camera: s5k5e2yb 20161004 BEGIN
static int OTP_Group;
// [SM31][Camera][akenhsu] 20161004 END

//#define CAPTURE_24FPS


static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K5E2YB_SENSOR_ID,
...
#define RG_TYPICAL 0x2EB
#define BG_TYPICAL 0x259
...
void S5K5E2YB_MIPI_read_otp_wb(struct S5K5E2YB_MIPI_otp_struct *otp)
{
	...
	kal_uint8 Structure_Version = 0; // Only for JIVE DP module, wrong Golden AWB data. 20161019 Joseph Hsu

    OTP_Group = 0;

	for(PageCount = 3; PageCount>=2; PageCount--)
	{
		LOG_INF("[OTP] PageCount=%d\n", PageCount);
        ...
        #if 1
        //====================
        LOG_INF("[OTP] PageCount=%x\n", PageCount);

        LOG_INF("[OTP] Flag=0x%x\n", read_cmos_sensor(0x0a04));
        LOG_INF("[OTP] Lens ID=0x%x\n", read_cmos_sensor(0x0a05));
        LOG_INF("[OTP] Module ID=0x%x\n", read_cmos_sensor(0x0a06));
        LOG_INF("[OTP] Structure Version=0x%x\n", read_cmos_sensor(0x0a07));
        LOG_INF("[OTP] AF_Macro[0:7]=0x%x\n", read_cmos_sensor(0x0a08));
        LOG_INF("[OTP] AF_Macro[8:15]=0x%x\n", read_cmos_sensor(0x0a09));
        LOG_INF("[OTP] AF_Infinity[0:7]=0x%x\n", read_cmos_sensor(0x0a0a));
        LOG_INF("[OTP] AF_Infinity[8:15]=0x%x\n", read_cmos_sensor(0x0a0b));
        LOG_INF("[OTP] AF_Starting[0:7]=0x%x\n", read_cmos_sensor(0x0a0c));
        LOG_INF("[OTP] AF_Starting[8:15]=0x%x\n", read_cmos_sensor(0x0a0d));
        LOG_INF("[OTP] AWB_R_Over_G=0x%x\n", read_cmos_sensor(0x0a0e));
        LOG_INF("[OTP] AWB_B_Over_G=0x%x\n", read_cmos_sensor(0x0a0f));
        LOG_INF("[OTP] AWB_Gr_Over_Gb=0x%x\n", read_cmos_sensor(0x0a10));
        LOG_INF("[OTP] Unit-AWB-R=0x%x\n", read_cmos_sensor(0x0a11));
        LOG_INF("[OTP] Unit-AWB-Gr=0x%x\n", read_cmos_sensor(0x0a12));
        LOG_INF("[OTP] Unit-AWB-Gb=0x%x\n", read_cmos_sensor(0x0a13));
        LOG_INF("[OTP] Unit-AWB-B=0x%x\n", read_cmos_sensor(0x0a14));
        LOG_INF("[OTP] Golden-AWB-R=0x%x\n", read_cmos_sensor(0x0a15));
        LOG_INF("[OTP] Golden-AWB-Gr=0x%x\n", read_cmos_sensor(0x0a16));
        LOG_INF("[OTP] Golden-AWB-Gb=0x%x\n", read_cmos_sensor(0x0a17));
        LOG_INF("[OTP] Golden-AWB-B=0x%x\n", read_cmos_sensor(0x0a18));
		...
		if(read_cmos_sensor(0x0a24) == 0x01)
		{
			...
			if(PageCount==3)
				LOG_INF("[S5K5E2YB] otp Group=3\n");
			else
				LOG_INF("[S5K5E2YB] otp Group=1\n");
			otp->module_integrator_id = read_cmos_sensor(0x0a26);

			Structure_Version = read_cmos_sensor(0x0a27); // Only for JIVE DP module, wrong Golden AWB data. 20161019 Joseph Hsu
		    ...
		    if (Structure_Version == 1) // Its AWB Golden data is wrong. Assign the correct one for BYD new IR. 20170222 Joseph Hsu
			{
				otp->golden_awb_r  = 0x73;
				otp->golden_awb_gr = 0xa0;
				otp->golden_awb_gb = 0xa0;
				otp->golden_awb_b  = 0x64;
			}
		...
		else if(read_cmos_sensor(0x0a04) == 0x01)
		{
			...
			if(PageCount==3)
				LOG_INF("[S5K5E2YB] otp Group=2\n");
			else
				LOG_INF("[S5K5E2YB] otp Group=0\n");
			otp->module_integrator_id = read_cmos_sensor(0x0a06);

			Structure_Version = read_cmos_sensor(0x0a07); // Only for JIVE DP module, wrong Golden AWB data. 20161019 Joseph Hsu
		...
		{
			LOG_INF("[S5K5E2YB] There is no OTP data \n");
		}
		write_cmos_sensor(0x0a00, 0x04);  //otp disable read
		write_cmos_sensor(0x0a00, 0x00);  //otp disable read
	}
	...
	if(PageCount == 1)
	{
		LOG_INF("[S5K5E2YB] otp all value is zero");
		write_cmos_sensor(0x0a00, 0x04);  //otp disable read
		write_cmos_sensor(0x0a00, 0x00);  //otp disable read
	}

	LOG_INF("[S5K5E2YB] Structure_Version=0x%x\n", Structure_Version);
	LOG_INF("[S5K5E2YB] otp->ModuleID=0x%x\n", otp->module_integrator_id);
	LOG_INF("[S5K5E2YB] otp->unit_awb_r=0x%x\n", otp->unit_awb_r);
	LOG_INF("[S5K5E2YB] otp->unit_awb_b=0x%x\n", otp->unit_awb_b);
	LOG_INF("[S5K5E2YB] otp->unit_awb_gr=0x%x\n", otp->unit_awb_gr);
	LOG_INF("[S5K5E2YB] otp->unit_awb_gb=0x%x\n", otp->unit_awb_gb);

	LOG_INF("[S5K5E2YB] otp->golden_awb_r=0x%x\n", otp->golden_awb_r);
	LOG_INF("[S5K5E2YB] otp->golden_awb_b=0x%x\n", otp->golden_awb_b);
	LOG_INF("[S5K5E2YB] otp->golden_awb_gr=0x%x\n", otp->golden_awb_gr);
	LOG_INF("[S5K5E2YB] otp->golden_awb_gb=0x%x\n", otp->golden_awb_gb);
}
...
void S5K5E2YB_MIPI_algorithm_otp_wb1(struct S5K5E2YB_MIPI_otp_struct *otp)
{
	...
	if((otp->unit_awb_r == 0) || (otp->unit_awb_b == 0) || (otp->unit_awb_gr == 0))
	{
	...
	LOG_INF("[S5K5E2YB] rg_ratio_unit=%d\n", rg_ratio_unit);
	LOG_INF("[S5K5E2YB] bg_ratio_unit=%d\n", bg_ratio_unit);
	...
	LOG_INF("[S5K5E2YB] bg_ratio_typical=%d\n", bg_ratio_typical);
	LOG_INF("[S5K5E2YB] rg_ratio_typical=%d\n", rg_ratio_typical);
	...
	LOG_INF("[S5K5E2YB] R_gain=0x%x\n", otp->R_Gain);
	LOG_INF("[S5K5E2YB] B_gain=0x%x\n", otp->B_Gain);
	LOG_INF("[S5K5E2YB] G_gain=0x%x\n", otp->G_Gain);
}
...
void S5K5E2YB_MIPI_write_otp_wb(struct S5K5E2YB_MIPI_otp_struct *otp)
{
	...
	LOG_INF("[S5K5E2YB] R_GainH=0x%x\n", R_GainH);
	LOG_INF("[S5K5E2YB] R_GainL=0x%x\n", R_GainL);
	LOG_INF("[S5K5E2YB] B_GainH=0x%x\n", B_GainH);
	LOG_INF("[S5K5E2YB] B_GainL=0x%x\n", B_GainL);
	LOG_INF("[S5K5E2YB] G_GainH=0x%x\n", G_GainH);
	LOG_INF("[S5K5E2YB] G_GainL=0x%x\n", G_GainL);
    ...
    {
#endif
        LOG_INF("[OTP] ====> write_cmos_sensor, OTP_Group = [0x%d] <==== \n", OTP_Group);
    ...
    {
        LOG_INF("[OTP] ====> no write OTP_Group = [0] <==== \n");
    }
#endif

    //2015/08/17 c -, reopen OTP AWB
	LOG_INF("[S5K5E2YB] [0x020e,0x%x]\n", read_cmos_sensor(0x020e));
	LOG_INF("[S5K5E2YB] [0x020f,0x%x]\n", read_cmos_sensor(0x020f));
	LOG_INF("[S5K5E2YB] [0x0210,0x%x]\n", read_cmos_sensor(0x0210));
	LOG_INF("[S5K5E2YB] [0x0211,0x%x]\n", read_cmos_sensor(0x0211));
	LOG_INF("[S5K5E2YB] [0x0212,0x%x]\n", read_cmos_sensor(0x0212));
	LOG_INF("[S5K5E2YB] [0x0213,0x%x]\n", read_cmos_sensor(0x0213));
	LOG_INF("[S5K5E2YB] [0x0214,0x%x]\n", read_cmos_sensor(0x0214));
	LOG_INF("[S5K5E2YB] [0x0215,0x%x]\n", read_cmos_sensor(0x0215));
}
...
//<2016/08/03-louisliu, Camera. Porting S5K5E2YB driver. Fix build error.
#if 0
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if(imgsensor.sensor_mode == IMGSENSOR_MODE_PREVIEW)
	{
		if((gain <= 944))
		{
			write_cmos_sensor(0x0b00, 0x01);//enable OTP shading
			LOG_INF("Preview enable OTP shading");
		}	
		else if( (gain >= 1024) )
		{
			write_cmos_sensor(0x0b00, 0x00);//disable OTP shading
			LOG_INF("Preview disable OTP shading");
		}
	}
	else
	{
		if((gain <= 944)  &&  (imgsensor.shutter < 9641))
		{
			write_cmos_sensor(0x0b00, 0x0107);//enable OTP shading
			LOG_INF("enable OTP shading");
		}	
		else if( (gain >= 1024) &&  (imgsensor.shutter >= 9641) )
		{
			write_cmos_sensor(0x0b00, 0x0000);//disable OTP shading
			LOG_INF("disable OTP shading");
		}
	}	
	#endif*/  
	return gain;
...
static kal_uint8 g_module_id = 0;
static kal_uint8 get_module_id(void)
{
	kal_uint16 PageCount;

	if (g_module_id != 0) return g_module_id;

	for(PageCount = 3 ; PageCount >= 1 ; PageCount--)
	{
		write_cmos_sensor(0x0a00, 0x04);
		write_cmos_sensor(0x0a02, PageCount); //otp enable read
		write_cmos_sensor(0x0a00, 0x01); //otp enable read

		g_module_id = read_cmos_sensor(0x0a06);
		LOG_INF("[OTP] PageCount=%d, Addr=0x%x, ModuleID=0x%x\n", PageCount, 0x0a06, g_module_id);
		if (g_module_id != 0) {
			write_cmos_sensor(0x0a00, 0x04);  //otp disable read
			write_cmos_sensor(0x0a00, 0x00);  //otp disable read
			break;
		...
		g_module_id = read_cmos_sensor(0x0a26);
		LOG_INF("[OTP] PageCount=%d, Addr=0x%x, ModuleID=0x%x\n", PageCount, 0x0a26, g_module_id);
		if (g_module_id != 0) {
			write_cmos_sensor(0x0a00, 0x04);  //otp disable read
			write_cmos_sensor(0x0a00, 0x00);  //otp disable read
			break;
	...
	LOG_INF("[OTP] Final ModuleID=0x%x\n", g_module_id);
	return g_module_id;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			LOG_INF("in while retry=%d SensorID=0x%x, sensor_info_id=0x%x, ModuleID=0x%x \n", retry, *sensor_id, imgsensor_info.sensor_id, module_id);
			if (*sensor_id == 0x5e20 && module_id == S5K5E2YB_MODULE_ID) {
				LOG_INF("Found S5K5E2 YB module! \n");
				*sensor_id = 0x5e21;
				break;
			} else {
	...
	if (*sensor_id == imgsensor_info.sensor_id) {
		...
		LOG_INF("Get SensorID OK SensorID=0x%x\n", *sensor_id);
	} else {
		// if Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF 
		*sensor_id = 0xFFFFFFFF;
		LOG_INF("Get SensorID XX SensorID=0x%x\n", *sensor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == 0x5e20) {
				if (module_id == S5K5E2YB_MODULE_ID) {
					LOG_INF("Found S5K5E2 YB module! \n");
					sensor_id = 0x5e21;
					LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
					break;
	...
	LOG_INF("SensorID=0x%x, sensor_info_id=0x%x, ModuleID=0x%x \n", sensor_id, imgsensor_info.sensor_id, module_id);
	if (module_id != S5K5E2YB_MODULE_ID) return ERROR_INVALID_DRIVER_MOD_ID;
