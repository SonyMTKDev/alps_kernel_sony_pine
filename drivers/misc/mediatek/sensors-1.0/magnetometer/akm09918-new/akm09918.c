...
static long AKECS_GetData(char *rbuf, int size)
{
	...
	if (loop_i >= AKM09918_RETRY_COUNT) {
		MAG_PR_ERR("Data read retry larger the max count!\n");
		if (0 == factory_mode)
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if (NULL == obj) {
		MAG_PR_ERR("akm09918_i2c_data is null!!\n");
		return 0;
	}
...
static int akm09918_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		MAG_PR_ERR("AKM09918 akm09918_probe: check device connect error\n");
	...
	if (err) {
		MAG_PR_ERR("akm09918_device register failed\n");
...
static int akm09918_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		MAG_PR_ERR("akm09918_delete_attr fail: %d\n", err);
