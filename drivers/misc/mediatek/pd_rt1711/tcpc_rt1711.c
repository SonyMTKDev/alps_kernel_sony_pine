...
#define RT1711_DRV_VERSION	"1.1.6_G"
...
/*[Arima_8100][bozhi_lin] dynamic check hw varient to set rt1711 IRQ 20170103 begin*/
enum board_version{
	board_pdp,
	board_dp,
	board_sp,
...
/*[Arima_8100][bozhi_lin] add rt1711 type-c show vendor id 20170109 begin*/
char *tcpc_show_vendor_id = NULL;
module_param(tcpc_show_vendor_id, charp, 0444);
...
static inline int rt1711_reset_ra_detach(struct tcpc_device *tcpc_dev)
{
	const uint8_t mask = RT1711_WAKEUP_EN |
		RT1711_CK_40K_SEL | RT1711_RA_DETACH_MASK;

	rt1711_i2c_write8(tcpc_dev, RT1711_REG_CC_EXT_CTRL, mask);
	return 0;
}

static inline int rt1711_check_ra_detach(struct tcpc_device *tcpc_dev)
{
	int ret;
	...
	ret = rt1711_i2c_read8(tcpc_dev, RT1711_REG_CC_EXT_CTRL);
	if (ret < 0)
		return ret;

	if (ret & RT1711_RA_DETACH_STATUS) {
		rt1711_reset_ra_detach(tcpc_dev);
		return 1;
	}
...
static int rt1711_init_alert_mask(struct tcpc_device *tcpc)
{
	...
	if (tcpc->tcpc_flags & TCPC_FLAGS_LPM_WAKEUP_WATCHDOG)
		mask |= RT1711_REG_ALERT_WAKEUP;
#endif	/* CONFIG_TYPEC_CAP_LPM_WAKEUP_WATCHDOG */

#ifdef CONFIG_TYPEC_CAP_RA_DETACH
	if (tcpc->tcpc_flags & TCPC_FLAGS_CHECK_RA_DETACHE)
		rt1711_reset_ra_detach(tcpc);
#endif	/* CONFIG_TYPEC_CAP_RA_DETACH */
...
/*[Arima_8100][bozhi_lin] dynamic check hw varient to set rt1711 IRQ 20170103 begin*/
#if 1
	if (chip->hw_version >= board_sp) {
		node = of_find_node_by_name(NULL, "type_c_port0");
	} else {
		node = of_find_node_by_name(NULL, "type_c_port0_under_sp");
	}
#else
	node = of_find_node_by_name(NULL, "type_c_port0");
#endif
...
static int rt1711_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	if (sw_reset) {
/*[Arima_8100][bozhi_lin] fix rt1711 will pull low VBUS with pd charger before shutdown 20170420 begin*/
#if 0
		rt1711_reg_write(chip->client, RT1711_REG_SWRESET, 1);
		mdelay(1);
#endif
/*[Arima_8100][bozhi_lin] 20170420 end*/
	}
...
#ifdef CONFIG_TYPEC_CAP_LPM_WAKEUP_WATCHDOG
	if (mask & TCPC_REG_ALERT_EXT_WAKEUP) {
		mask_t &= ~TCPC_REG_ALERT_EXT_WAKEUP;
		mask_t |= RT1711_REG_ALERT_WAKEUP;
	}
#endif	/* CONFIG_TYPEC_CAP_LPM_WAKEUP_WATCHDOG */
...
#ifdef CONFIG_TYPEC_CAP_LPM_WAKEUP_WATCHDOG
	if (ret & RT1711_REG_ALERT_WAKEUP)
		*alert |= TCPC_REG_ALERT_EXT_WAKEUP;
#endif	/* CONFIG_TYPEC_CAP_LPM_WAKEUP_WATCHDOG */

#ifdef CONFIG_TYPEC_CAP_RA_DETACH
	if (rt1711_check_ra_detach(tcpc) > 0)
...
{
	int rv = rt1711_i2c_read8(tcpc_dev, RT1711_REG_LOW_POWER_CTRL);
...
/*[Arima_8100][bozhi_lin] dynamic check hw varient to set rt1711 IRQ 20170103 begin*/
#if 1
	chip->pcba_ver0 = of_get_named_gpio(np, "rt1711,pcba_ver0", 0);
	pr_info("%s chip->pcba_ver0 = %d\n", __func__, chip->pcba_ver0);
	chip->pcba_ver1 = of_get_named_gpio(np, "rt1711,pcba_ver1", 0);
	pr_info("%s chip->pcba_ver1 = %d\n", __func__, chip->pcba_ver1);
	chip->pcba_ver2 = of_get_named_gpio(np, "rt1711,pcba_ver2", 0);
	pr_info("%s chip->pcba_ver2 = %d\n", __func__, chip->pcba_ver2);

	if(__gpio_get_value(chip->pcba_ver0) && __gpio_get_value(chip->pcba_ver1) && __gpio_get_value(chip->pcba_ver2)) {
		chip->hw_version = board_pdp;
		pr_info("[B]%s(%d): PDP\n", __func__, __LINE__);
	} else if(!__gpio_get_value(chip->pcba_ver0) && __gpio_get_value(chip->pcba_ver1) && __gpio_get_value(chip->pcba_ver2)) {
		chip->hw_version = board_dp;
		pr_info("[B]%s(%d): DP\n", __func__, __LINE__);
	} else if(__gpio_get_value(chip->pcba_ver0) && !__gpio_get_value(chip->pcba_ver1) && __gpio_get_value(chip->pcba_ver2)) {
		chip->hw_version = board_sp;
		pr_info("[B]%s(%d): SP\n", __func__, __LINE__);
	} else {
		chip->hw_version = board_sp;
		pr_info("[B]%s(%d): after SP\n", __func__, __LINE__);
	}

	if (chip->hw_version >= board_sp) {
		chip->irq_gpio = of_get_named_gpio(np, "rt1711,irq_gpio", 0);
		pr_info("[B]%s(%d): chip->irq_gpio = %d\n", __func__, __LINE__, chip->irq_gpio);
	} else {
		chip->irq_gpio = of_get_named_gpio(np, "rt1711,irq_gpio_under_sp", 0);
		pr_info("[B]%s(%d): chip->irq_gpio = %d\n", __func__, __LINE__, chip->irq_gpio);
	}
#else
	chip->irq_gpio = of_get_named_gpio(np, "rt1711,irq_gpio", 0);
...
/*[Arima_8100][bozhi_lin] add rt1711 type-c show vendor id 20170109 begin*/
	{
		char buf[20]={0};
		sprintf(buf, "RT1711_0x%04X", chip->ven_id);
		if (tcpc_show_vendor_id == NULL) {
			tcpc_show_vendor_id = kmalloc(strlen(buf) + 1, GFP_KERNEL);
		}
		if (tcpc_show_vendor_id != NULL) {
			strcpy(tcpc_show_vendor_id, buf);
		}
	}
/*[Arima_8100][bozhi_lin] 20170109 end*/
...
{
/*[Arima_8100][bozhi_lin] fix type-c rt1711 kernel panic 20170104 begin*/
#if 0
...
#endif
/*[Arima_8100][bozhi_lin] 20170104 end*/
	return 0;
}
...
/* RT1711 Porting Guide
 *
 * A. DTSI Files
 *
 *     1. rt1711 i2c device, please add usb_type_c node @ i2c bus
 *
 *     i2c1: i2c@11008000 {
 ...
 *             rt1711,irq_gpio = <&pio 7 0x0>; // gpio number for Interrupt
 *     };
 *
 * B. EINT : please add EINT for RT1711 Interrupt
 ...
 *
 * C. Kconfig
 *     please open these options for RT1711 Type-C Only Driver
 *
 *     CONFIG_RT_REGMAP=y
