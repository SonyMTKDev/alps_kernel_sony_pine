...
static int tpd_allow_upgrade(unsigned char tpd_fw_ver, unsigned char host_fw_ver)
{
	...
	return 0;
#else
	if (tpd_fw_ver != host_fw_ver) //20160909 Bozhi, need to change rule
		return 1;
	return 0;
#endif
}
...
/* add end */
	...
	ret = tpd_i2c_write_interface(client, auc_i2c_write_buf, 3);
	msleep(10);
	tpd_byte_read(client, reg_val, 3);
	TPD_DMESG("reg_val[0]=0x%x, reg_val[1]=0x%x, reg_val[2]=0x%x\n", reg_val[0], reg_val[1], reg_val[2]);
	...
	} while (reg_val[0] != 0x54 || reg_val[1] != 0x2C);
/*
#ifdef FT5X36_UPGADE
	...
	for(i = 0; i < 15; i++)
	{
		...
		ft5x0x_i2c_Read(client, auc_i2c_write_buf, 1, reg_val, 2);
	...
	tpd_i2c_write_interface(client, auc_i2c_write_buf, 4);
#else
...
/* tpd_cmd_write(client, 0xCC, 0x00, 0x00, 0x00, 1); */
	...
	TPD_DMESG("[TPD] checksum_val=0x%x, bt_ecc=0x%x\n", val, bt_ecc);
	if (val != bt_ecc) {
...
/* msleep(300); //make sure CTP startup normally */
	...
	tpd_i2c_write_interface(client, auc_i2c_write_buf, 1);
	msleep(300);
