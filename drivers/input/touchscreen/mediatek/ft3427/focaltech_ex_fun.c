...
/*[Arima_8100][bozhi_lin] touch aats test implement 20161115 begin*/
static ssize_t fts_test_result_show(struct device *dev,
...
static ssize_t fts_test_result_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	int store_test_result;
	if (kstrtouint(buf, 10, &store_test_result) == 0) {
		if (store_test_result == 1) {
			test_result = 1;
		} else {
			test_result = 0;
		}
