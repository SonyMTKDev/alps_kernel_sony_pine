...
//20170104 patch for Arima SM31 accuracy tuning
	data[0] = 0x0F;
	data[1] = 0xE1;
	I2C_TxData(data, 2);
 ...
 */
int TEST_DATA_1(const char testno[], const char testname[], const int testdata,
...
/*!
 ...
 @retval 0 The test is aborted by kind of system error.
 */
int FctShipmntTestProcess_Body_1(void)
{
	int pf_total = 1;
	...
	/* Reset Test Result */
	/* *********************************************** */
	TEST_DATA_1(NULL, "START", 0, 0, 0, &pf_total);
	...
	/* Judge Test Result */
	/* *********************************************** */
	TEST_DATA_1(NULL, "END", 0, 0, 0, &pf_total);
...
static ssize_t show_shipment_test(struct device_driver *ddri, char *buf)
{
	...
	res = FctShipmntTestProcess_Body_1();
	if (1 == res) {
	...
	long ret = 0;
		...
		switch(cmd)
		{
			...
			case COMPAT_MMC31XX_IOC_RESET:
				...
				break;	//fix coverity issues, 161103
...
static int mmc3630x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->hw = hw;
	data->hw->direction = 5; //change direction for SM31
	...
	data->client = client;
	data->client->addr = 0x30;	//forced change i2c slave addr
	printk("[mmc3630x] kernel: auto probe address 0x30\n");
	...
	if(temp < 0)
	{
		...
		//read again
		temp = I2C_RxData(product_id, 1);	//fix coverity issues, 161103
	}
	
	printk("mmc3630x_temp=%d, product_id[0] = %d\n",temp,product_id[0]);
	if(product_id[0] != 10 || (temp < 0))	//fix coverity issues, 161103
	{
	      goto exit_kfree;
	}
...
//20170104 patch for Arima SM31 accuracy tuning
	...
	{
		printk(KERN_ERR "mmc3630x_create attribute err = %d\n", err);
	...
	if(err)
	{
		printk(KERN_ERR "mmc3630x_attach fail = %d\n", err);
		goto exit_kfree;
	}
	...
	MMCDBG("%s: OK\n", __func__);
	printk("mmc3630x_IIC probe successful !");
