...
#if defined(ARIMA_FT_TOUCH_PORTING)
...
unsigned int tpd_int_gpio_number = 1;  //mtk_tpd.c (tpd_gpio_output)
...
#endif
struct touch_info {
	int y[TPD_SUPPORT_POINTS];      //Y Position
	int x[TPD_SUPPORT_POINTS];      //X Position
	int p[TPD_SUPPORT_POINTS];      //Event Flag
	int id[TPD_SUPPORT_POINTS];     //ID
	int weight[TPD_SUPPORT_POINTS]; //WEIGHT
	int area[TPD_SUPPORT_POINTS];   //AREA
	int count;
};
 ...
 */
s32 ft5x0x_i2c_read(u16 addr, u8 *buffer, s32 len)
{
#if TPD_SUPPORT_I2C_DMA
 ...
 *          which contains the error code.
 */
s32 ft5x0x_i2c_write(u16 addr, u8 *buffer, s32 len)
{
#if TPD_SUPPORT_I2C_DMA
...
static void tpd_down(int x, int y, int p, int id, int weight, int area)
{
	...
	{
#endif
		TPD_DEBUG("%s x:%d y:%d p:%d, id:%d, weight:%d, area:%d\n", __func__, x, y, p, id, weight, area);
		input_report_key(tpd->dev, BTN_TOUCH, 1);
		input_report_abs(tpd->dev, ABS_MT_TOUCH_MAJOR, area);
		input_report_abs(tpd->dev, ABS_MT_POSITION_X, x);
		input_report_abs(tpd->dev, ABS_MT_POSITION_Y, y);
		input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, id);
		input_report_abs(tpd->dev, ABS_MT_PRESSURE, weight);
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
...
#if defined(ARIMA_FT_TOUCH_PORTING)
	ft5x0x_i2c_read(0x00, &(data[0]), 8);
	ft5x0x_i2c_read(0x08, &(data[8]), 8);
	ft5x0x_i2c_read(0x10, &(data[16]), 8);
	ft5x0x_i2c_read(0x18, &(data[24]), 8);
	ft5x0x_i2c_read(0xa6, &(data[32]), 1);
	ft5x0x_i2c_read(0x88, &report_rate, 1);
#else
	...
	if (report_rate < 8) {
		report_rate = 0x8;
#if defined(ARIMA_FT_TOUCH_PORTING)
		if ((ft5x0x_i2c_write(0x88, &report_rate, 1)) < 0)
#else
	...
	for (i = 0; i < cinfo->count; i++) {
		...
		/*get the WEIGHT, 2 bytes*/
		weight = data[3 + 6 * i + 4];
		cinfo->weight[i] = weight;

		/*get the AREA, 1 bytes*/
		area = data[3 + 6 * i + 5];
		area >>= 4;
		area &= 0x000F;
		cinfo->area[i] = area;

		TPD_DEBUG("cinfo->x[%d] = %d, cinfo->y[%d] = %d, cinfo->p[%d] = %d, cinfo->id[%d] = %d, cinfo->weight[%d] = %d, cinfo->area[%d] = %d\n", i,
		cinfo->x[i], i, cinfo->y[i], i, cinfo->p[i], i, cinfo->id[i], i, cinfo->weight[i], i, cinfo->area[i]);
	}
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		if (tpd_touchinfo(&cinfo, &pinfo)) {
			...
			if (cinfo.count > 0) {
				for (i = 0; i < cinfo.count; i++)
					tpd_down(cinfo.x[i], cinfo.y[i], cinfo.p[i], cinfo.id[i], cinfo.weight[i], cinfo.area[i]);
			} else {
#ifdef TPD_SOLVE_CHARGING_ISSUE
				tpd_up(1, 48);
#else
...
#if 1 //mtk_tpd.c (tpd_gpio_as_int)
...
#if defined(ARIMA_FT_TOUCH_PORTING)
	if ((ft5x0x_i2c_read(0x00, &data, 1)) < 0) {
#else
...
static s8 ftp_enter_doze(struct i2c_client *client)
{
...
#if defined(ARIMA_FT_TOUCH_PORTING)
	ret = ft5x0x_i2c_write(FT_GESTRUE_MODE_SWITCH_REG, &gestrue_on, 1);
#else
	...
	for (i = 0; i < 10; i++) {
#if defined(ARIMA_FT_TOUCH_PORTING)
		ft5x0x_i2c_read(FT_GESTRUE_MODE_SWITCH_REG, &gestrue_data, 1);
#else
		...
		msleep(20);
#if defined(ARIMA_FT_TOUCH_PORTING)
		ret = ft5x0x_i2c_write(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, &gestrue_on, 1);
#else
		ret = i2c_smbus_write_i2c_block_data(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, 1, &gestrue_on);
#endif
...
static void tpd_suspend(struct device *h)
{
	...
	if (tpd_scp_doze_en) {
		...
		disable_irq(touch_irq);
#if defined(ARIMA_FT_TOUCH_PORTING)
		ft5x0x_i2c_write(0xA5, &data, 1);  /* TP enter sleep mode */
#else
		i2c_smbus_write_i2c_block_data(i2c_client, 0xA5, 1, &data);  /* TP enter sleep mode */
#endif
